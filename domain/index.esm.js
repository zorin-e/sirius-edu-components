...
function hasSlotContent(slot) {
    ...
    return content.findIndex((s) => s.type !== Comment) !== -1;
}
...
function isMobile() {
    ...
    result =
        firstRegex.test(userAgent.toLowerCase()) ||
            secondRegex.test(userAgent.substring(0, 4).toLowerCase());
    computed = true;
    return result;
}
...
export { clamp, dasherize, getClipboardDataText, getPhoneNumber, hasSlotContent, isMobile };
